...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('form_surat', function (Blueprint $table) {
            $table->id();
            $table->foreignId('form_id')->constrained('sub_surat_type')->onDelete('cascade');
            $table->string('name');      // nama field, misal "nama_kepala_keluarga"
            $table->string('label');     // label field, misal "Nama Kepala Keluarga"
            $table->enum('type', ['text','select','date']); // tipe input
            $table->boolean('required')->default(false);
            $table->json('options')->nullable(); // opsi kalau select
            $table->timestamps();
        });
    }
