...
<div class="p-4 bg-white rounded shadow dark:text-gray-800">
    ...
    <div class="header text-center mb-4">
        <div class="title text-xs font-bold underline mt-4 mb-1" style="text-transform: uppercase">
            {{ $record->subSuratType->suratType->nama_surat ?? 'SURAT KETERANGAN BEDA NAMA' }}
        </div>
    ...
    <div class="content text-justify text-xs">
        ...
        <p class="mb-2 text-xs">Menerangkan bahwa terdapat kesalahan nama pada {{ $record->data_surat['keperluan'] }} dengan NIK {{ $record->data_surat['nik'] }}. Berikut ini identitas diri sesuai dengan {{ $record->data_surat['keperluan'] }} yang sebenarnya: </p>

        <div class="identity my-4 text-xs">
            <div class="flex mb-1">
                <div class="w-32">Nama</div>
                <div class="w-4">:</div>
                <div>{{ $record->data_surat['nama_seharusnya'] ?? '-' }}</div>
            </div>
            <div class="flex mb-1">
                <div class="w-32">NIK</div>
                <div class="w-4">:</div>
                <div>
                    {{ $record->data_surat['nik'] }}
                </div>
            </div>
            <div class="flex mb-1">
                <div class="w-32">Tempat/tanggal lahir</div>
                <div class="w-4">:</div>
                <div>
                    @if (isset($record->data_surat['tempat_lahir']) &&
                    isset($record->data_surat['tanggal_lahir']))
                    {{ ucfirst($record->data_surat['tempat_lahir']) . '/' . $record->data_surat['tanggal_lahir'] }}
                    @else
                    -
                    @endif
                </div>
            ...
            <div class="flex mb-1">
                <div class="w-32">Alamat</div>
                <div class="w-4">:</div>
                <div>{{ $record->data_surat['alamat'] ?? '-' }}</div>
            </div>
            <div class="flex mb-1">
                <div class="w-32">Ibu Kandung</div>
                <div class="w-4">:</div>
                <div>{{ $record->data_surat['ibu_kandung'] ?? '-' }}</div>
            </div>
        </div>
    </div>
